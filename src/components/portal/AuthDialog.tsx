      ...
      <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl">
              <div className="w-8 h-8 rounded-lg bg-[#1f8f15]/10 flex items-center justify-center">
                <Shield className="h-4 w-4 text-[#1f8f15]" />
              </div>
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="flex justify-center gap-2">
              {otpDigits.map((digit, index) => (
                <Input
                  ...
                  className="w-10 h-12 text-center text-lg font-bold border-2 focus:border-[#1f8f15] rounded-lg"
                  autoFocus={index === 0}
                />
              ))}
            </div>
            ...
            <Button className="w-full bg-[#1f8f15] hover:bg-[#2d7a28]" onClick={handle2FAVerification} disabled={loading}>
      ...
      <DialogContent className="sm:max-w-[480px] max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as 'login' | 'register')}>
          ...
          <TabsContent value="login" className="space-y-4 mt-4">
            ...
            <Button className="w-full bg-[#1f8f15] hover:bg-[#2d7a28]" onClick={handleLogin} disabled={loading}>
            ...
            <div className="bg-[#1f8f15]/5 rounded-lg p-3 text-sm">
              <p className="font-medium text-[#1f8f15] mb-2">Quick {envLabel} Access:</p>
              <div className="space-y-1.5">
                {Object.entries(credentials).map(([role, cred]) => {
                  const Icon = demoIconMap[role] || Users
                  return (
                    <button key={role} onClick={() => fillDemo(role as DemoRole)} className="flex items-center gap-2 text-[#1f8f15] hover:text-[#2d7a28] w-full text-left">
                      <Icon className="h-3.5 w-3.5" /> {cred.label}: {cred.email}
                    </button>
                  )
                })}
              </div>
              <p className="text-[#1f8f15]/70 mt-1">Password: demo123</p>
            </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="register" className="space-y-4 mt-4">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {roles.map((role) => (
                  <Card key={role.value}
                    className={`cursor-pointer transition-all ${selectedRole === role.value ? `border-[#1f8f15] bg-[#1f8f15]/5 ring-1 ring-[#1f8f15]` : 'border-gray-200 hover:border-gray-300'}`}
                    onClick={() => setSelectedRole(role.value)}>
                    <CardContent className="p-3 text-center">
                      <role.icon className={`h-6 w-6 mx-auto mb-1 ${selectedRole === role.value ? 'text-[#1f8f15]' : 'text-gray-400'}`} />
                      <div className={`text-xs font-medium ${selectedRole === role.value ? 'text-[#1f8f15]' : 'text-gray-600'}`}>
                        {role.label}
                      </div>
                    </CardContent>
                  </Card>
            ...
            <Button className="w-full bg-[#1f8f15] hover:bg-[#2d7a28]" onClick={handleRegister} disabled={loading}>
